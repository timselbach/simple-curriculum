...
from simple_project.training.metrics import SentenceEvaluator
from simple_project.data.load_data import get_articles_with_level, read_articles, prep_text

os.environ['TOKENIZERS_PARALLELISM'] = 'true'

ALL_DATASETS = ["OSE", "SimpleWiki", "SimpleGerman"]
PATHS = {
    "OSE": "C:/Users/timse/Documents/Test repo/simple-curriculum/datasets/OneStopEnglishCorpus/data/",
    "Simple": "C:/Users/timse/Documents/Test repo/simple-curriculum/datasets/SimpleWikipedia/data/",
    "SimpleGerman": "C:/Users/timse/Documents/Test repo/simple-curriculum/datasets/SimpleGerman/Datasets/"
}
...
class SimpleWikiDataset(BaseDataset):
    def __init__(self, path: Union[str, None] = None, level: str = None, *args, **kwargs):
        ...
        if self.split == "train":
            ...
            self.data = self.all_sentences if not self.level else self._filter_by_level(self.all_sentences)
        else:
    ...
    def _create_simple_wiki_dataset(self, parallel: bool) -> list:
        ...
        parallel = 5 if parallel else None
...
class OSEDataset(BaseDataset):
    ...
    def _create_ose_dataset(self) -> list:
        ...
        parallel = 5 if self.parallel else None
        # big work happens here:
        results = evaluator.evaluate(sentences)
...
def get_classification_xy(dataset, balanced: bool = True, level: list = None, return_sents: bool = False):
    ...
    Args:
        ...
        balanced (bool): Whether to balance the classes by discarding excess samples.
        level (list): Specific levels to include; if None, includes all levels.
        return_sents (bool): Whether to return the list of sentences.

    Returns:
        X (np.ndarray): Feature matrix.
        y (np.ndarray): Labels.
        sentences (list, optional): List of sentences corresponding to each sample.
    """
    # Define the mapping from level to label based on dataset name
    if dataset.name == 'OSE':
        ...
        raise NotImplementedError(f"Dataset name '{dataset.name}' not supported.")

    np.random.seed(dataset.seed)

    # If no specific levels are provided, include all levels
    if level is None:
        selected_levels = list(level_to_label.values())
    else:
    ...
    X = []
    y = []
    sentences = [] if return_sents else None

    # Iterate through the dataset and collect X, y, and sentences
    for entry in dataset:
        entry_level_str = entry["level"]
        if entry_level_str not in level_to_label:
            print(f"Skipping entry with unknown level: {entry_level_str}")
            continue  # Skip entries with levels not in the mapping
        lbl = level_to_label[entry_level_str]
        if lbl not in selected_levels:
            continue  # Skip if label is not in selected levels
    ...
    print(f"Label distribution before balancing: {Counter(y)}")
    if return_sents:
    ...
    if balanced:
        ...
        try:
            min_samples = min(label_counts[l] for l in selected_levels if l in label_counts)
        except ValueError:
            raise ValueError("No samples found for the selected levels.")

        print(f"Minimum number of samples per class for balancing: {min_samples}")

        balanced_X = []
        balanced_y = []
        balanced_sentences = [] if return_sents else None

        for l in selected_levels:
            # Find all indices for class l
            class_indices = np.where(y == l)[0]
            if len(class_indices) < min_samples:
                raise ValueError(
                    f"Not enough samples for class {l} to balance. Required: {min_samples}, Found: {len(class_indices)}")

            # Shuffle class indices to ensure random sampling
            np.random.shuffle(class_indices)

            # Select the first min_samples indices after shuffling
            selected_indices = class_indices[:min_samples]

            # Append the selected samples to balanced lists
            balanced_X.append(X[selected_indices])
            balanced_y.append(y[selected_indices])
            if return_sents:
                balanced_sentences.extend([sentences[i] for i in selected_indices])

        # Concatenate balanced class subsets
        X_balanced = np.concatenate(balanced_X)
        y_balanced = np.concatenate(balanced_y)
        if return_sents:
            sentences_balanced = balanced_sentences

        # Debug: Print counts after balancing
        print(f"Total samples after balancing: {len(X_balanced)}")
        print(f"Label distribution after balancing: {Counter(y_balanced)}")
        if return_sents:
            print(f"Total sentences after balancing: {len(sentences_balanced)}")
        ...
        if return_sents:
            sentences_balanced = [sentences_balanced[i] for i in shuffle_indices]

        # Verify alignment
        if return_sents:
            assert len(X_balanced) == len(y_balanced) == len(sentences_balanced), \
                f"Lengths do not match: X={len(X_balanced)}, y={len(y_balanced)}, sentences={len(sentences_balanced)}"
            return X_balanced, y_balanced, sentences_balanced
        else:
            assert len(X_balanced) == len(y_balanced), \
                f"Lengths do not match: X={len(X_balanced)}, y={len(y_balanced)}"
            return X_balanced, y_balanced
    else:
        # If not balancing, just return the collected data
        if return_sents:
            assert len(X) == len(y) == len(sentences), \
                f"Lengths do not match: X={len(X)}, y={len(y)}, sentences={len(sentences)}"
            return X, y, sentences
        else:
            assert len(X) == len(y), \
                f"Lengths do not match: X={len(X)}, y={len(y)}"
            return X, y
